package co.edu.uco.quotes.dto;

import java.util.ArrayList;
import java.util.List;

import co.edu.uco.crosscutting.util.numeric.UtilNumeric;
import co.edu.uco.crosscutting.util.object.UtilObject;
import co.edu.uco.crosscutting.util.text.UtilText;

public class EmployeeTypeDTO {
	private int id;
	private String name;
	private int salary;
	private String function;

	
	public EmployeeTypeDTO() {
		super();
		setFunction(UtilText.EMPTY);
		setName(UtilText.EMPTY);
		setSalary(0);
	}
	public EmployeeTypeDTO(int id, String name, int salary, String function) {
		super();
		setFunction(function);
		setId(id);
		setName(name);
		setSalary(salary);
	}
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getName() {
		return name;
	}
	public void setName(String name) {
		this.name = UtilText.getDefault(name);
	}
	public int getSalary() {
		return salary;
	}
	public void setSalary(int salary) {
		this.salary = (int) UtilNumeric.getUtilNumeric().getDefault(salary);
	}
	public String getFunction() {
		return function;
	}
	public void setFunction(String function) {
		this.function = UtilText.getDefault(function);
	}
	
	public void validateName(List<String> validationMessages) {

		validationMessages = UtilObject.getUtilObject().getDefault(validationMessages, new ArrayList<>());

		if (UtilText.isEmpty(getName())) {

			validationMessages.add("Name of EmployeeType is required");

		} else if (UtilText.getDefault(getName()).length() > 50) {

			validationMessages.add("The lenght of the name of id type must be less than 50 charecters");

		} else if (!UtilText.getDefault(getName()).matches("^[a-zA-ZÒ—·¡È…ÌÕÛ”˙⁄ ]*$")) {
			validationMessages.add("The name of id type has not valid characters");
		}
	}
	
	public void validateId(List<String> validationMessages) {
		validationMessages = UtilObject.getUtilObject().getDefault(validationMessages, new ArrayList<>());
		if(!UtilNumeric.getUtilNumeric().isGreatherThan(getId(), 0)) {
			validationMessages.add("The id must be greather than zero");
		}
		
	}
	
	public void validateSalary(List<String> validationMessages) {
		validationMessages = UtilObject.getUtilObject().getDefault(validationMessages, new ArrayList<>());
		if(!UtilNumeric.getUtilNumeric().isGreatherThan(getSalary(), 0)) {
			validationMessages.add("The salary must be greather than zero");
		}
		
	}
	
	public void validateFunction(List<String> validationMessages) {

		validationMessages = UtilObject.getUtilObject().getDefault(validationMessages, new ArrayList<>());

		if (UtilText.isEmpty(getName())) {

			validationMessages.add("Function of employee type is required");

		} else if (UtilText.getDefault(getFunction()).length() > 100) {

			validationMessages.add("The lenght of the name of id type must be less than 100 charecters");

		}
	}
	
	
}
